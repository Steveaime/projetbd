...
def RemoveAcents(text): # fonction permettant de retirer les blancs , retirer les accents , de transformer le textes en majuscule et de concatener le tout
    try:
...
def LectureMessage(file): # fonction permettant de lire le message dans le ficheir texte 
...
def caesarShift(str, key): # chiffremenr de cesar 
	output = ""
...
def SauvegardeMessage(file, message): # fonction permettant de sauvegarder le texter chiffrer au prealable 
    fs = open(file, 'w') 
...
def Menu(): # fonction permetant d'afficher le menu et de selectionner une option 
...
def Egnima(rotors_file,rotors, messageIN): # le couer du programme la fonction de chiffrement/dechiffrement egnima 
